...
project_path = "Q-learning-control"
...
sumoCmd = [
    ...
    cfg_path,
    "--tripinfo-output",
    project_path + "/tripinfo2_TEST.xml",
...
def output_lane_speed():  # 输出01234道路的平均速度
    ...
    car_index = []
    car_speed = {
        "H_0": H_0_meanspeed_list,
        "H_1": H_1_meanspeed_list,
        "H_2": H_2_meanspeed_list,
        "H_3": H_3_meanspeed_list,
        "H_4": H_4_meanspeed_list,
    }
    ...
    car_simple_speed.to_csv(
        r"SUMO-RL-ramp-control/Q-learning-control/csv/" + T1 + ".csv"
    )
    ...
    plt.show()
    fig.savefig(r"SUMO-RL-ramp-control/Q-learning-control/img/" + T1 + ".png")
    ...
    plt.show()
    fig.savefig(
        r"F:/大学/MTFwiki\SUMO-Q_learning\SUMO-RL-ramp-control/Q-learning-control/img/"
        + T1
        + "_box.png"
    )
    ...
    plt.legend(fontsize=20)
    plt.show()
    fig.savefig(r"SUMO-RL-ramp-control/Q-learning-control/img/" + T2 + ".png")
...
def output_lane_OOC():  # 画图
    ...
    for i in range(0, get_OOC.count()[0]):
        if i % 20 == 0:
            ...
            car_OOC_simpleall.append(
                (get_OOC["H_0"][i] + get_OOC["H_1"][i] + get_OOC["H_2"][i]) / 3
            )
    ...
    car_simple_OOC = {
        ...
        "H_3": car_OOC_simple4,
        "H_4": car_OOC_simple5,
        "H_all": car_OOC_simpleall,
    }
    car_simple_OOC = pd.DataFrame(data=car_simple_OOC, index=car_OOC_index)
    car_simple_OOC.to_csv(r"SUMO-RL-ramp-control/Q-learning-control/csv/" + T3 + ".csv")
    ...
    plt.show()
    fig.savefig(r"SUMO-RL-ramp-control/Q-learning-control/img/" + T3 + ".png")
    ...
    plt.show()
    fig.savefig(r"SUMO-RL-ramp-control/Q-learning-control/img/" + T3 + "_box.png")
    ...
    plt.legend(fontsize=20)
    plt.show()
    fig.savefig(r"SUMO-RL-ramp-control/Q-learning-control/img/" + T4 + ".png")
...
def traci_control(step_time):
    ...
    for step in range(0, step_time):
        ...
        get_OOCall_list.append(
            (
                traci.lane.getLastStepOccupancy("H_0")
                + traci.lane.getLastStepOccupancy("H_1")
                + traci.lane.getLastStepOccupancy("H_2")
                + traci.lane.getLastStepOccupancy("H_3")
                + traci.lane.getLastStepOccupancy("H_4")
            )
            / 5
            * 100
        )
        ...
        traci.trafficlight.setRedYellowGreenState(
            traci.trafficlight.getIDList()[0], "g" + "G"
        )  # trafficlight_control(step)  trafficlight_control2(step)
...
if __name__ == "__main__":
    ...
    print(qq_table)
    qq_table.to_excel(
        r"F:\software two\sumo-1.8.0/file1/img/" + "qqtable.xlsx", index=False
    )
