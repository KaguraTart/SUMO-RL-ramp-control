...
sumoCmd = [sumoBinary, "-c", r"F:\software two\sumo-1.8.0/file3/beihong2.sumo.cfg",'--tripinfo-output',r'F:\software two\sumo-1.8.0/file3/tripinfo2.xml','--duration-log.statistics']
...
def output_lane_speed():   #输出01234道路的平均速度
    ...
    car_speed = {'H_0':H_0_meanspeed_list,
        'H_1':H_1_meanspeed_list,'H_2':H_2_meanspeed_list,'H_3':H_3_meanspeed_list}
    ...
    car_simple_speed = {'H_0':car_simple1,'H_1':car_simple2,'H_2':car_simple3 ,'H_3':car_simple4}
    car_simple_speed = pd.DataFrame(data = car_simple_speed,index = car_index)
    ax = car_simple_speed[['H_0', 'H_1','H_2','H_3']].plot(fontsize =30)
    plt.title('Lane car mean speed for NONE control ',fontsize = 30)
    fig = ax.get_figure()
    plt.xlabel('time /min',fontsize = 30)
    plt.ylabel('speed km/h',fontsize = 30)
    plt.show()
    fig.savefig(r'F:\software two\sumo-1.8.0/file3/img1/'   + 'car_speed_lane_NONE.png')
    ...
    plt.title('car mean speed of H_1 and H_2 for NONE control',fontsize = 25)
    fig = ax.get_figure()
    plt.xlabel('time /min',fontsize = 30)
    plt.ylabel('speed km/h',fontsize = 30)
    plt.show()
    fig.savefig(r'F:\software two\sumo-1.8.0/file3/img1/'   + 'car_mean_speed12_NONE.png')

def output_lane_OOC(): #画图
    get_OOC = {'H_0':get_OOC0_list,
        'H_1':get_OOC1_list,'H_2':get_OOC2_list,'H_3':get_OOC3_list}
    ...
    for i in range(0,get_OOC.count()[0]):
        if i % 20 == 0:
            ...
            car_OOC_simpleall.append((get_OOC['H_0'][i]+get_OOC['H_1'][i]+get_OOC['H_2'][i]+get_OOC['H_3'][i])/4)
            car_OOC_index.append(i/60)
    car_simple_OOC = {'H_0':car_OOC_simple1,'H_1':car_OOC_simple2,'H_2':car_OOC_simple3 ,'H_3':car_OOC_simple4,'H_all':car_OOC_simpleall}
    car_simple_OOC = pd.DataFrame(data = car_simple_OOC,index = car_OOC_index)
    ax = car_simple_OOC[['H_0', 'H_1','H_2','H_3']].plot(fontsize =30)
    plt.title('Lane car OCC for NONE control',fontsize = 30)
    fig = ax.get_figure()
    plt.xlabel('time /min',fontsize = 30)
    plt.ylabel('%',fontsize = 30)
    plt.show()
    fig.savefig(r'F:\software two\sumo-1.8.0/file3/img1/'   + 'OCC_NONE.png')

    ax = car_simple_OOC[['H_all']].plot(fontsize =30)
    plt.title('Lane car OCC_mean for NONE control ',fontsize = 30)
    fig = ax.get_figure()
    plt.xlabel('time /min',fontsize = 30)
    plt.ylabel('%',fontsize = 30)
    plt.show()
    fig.savefig(r'F:\software two\sumo-1.8.0/file3/img1/'   + 'OCCmean_NONE.png')
